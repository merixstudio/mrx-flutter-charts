...
class _ChartGroupPiePainter {
  ...
  static void draw({
    ...
    for (int i = 0; i < layer.items.length; i++) {
      ...
      final double gapSweepSize =
          items.length < 2 ? 0.0 : layer.settings.gapSweepAngle * items.length;
      final double sliceSweepTotal = 360.0 - gapSweepSize;
      final double total = items.map((e) => e.amount).sum;
      _sliceCursor =
          layer.settings.angleOffset + layer.settings.gapSweepAngle.half;
      final double itemRadius =
          radius + i * layer.settings.gapBetweenChartCircles;
      ...
      for (int j = 0; j < items.length; j++) {
        ...
        final double strokeAngle =
            twoPi * layer.settings.thickness / circumference;
        touchableShapes.add(
  ...
  static void _calculate({
    ...
    item.setupValue(
      ...
      initialStartAngle: oldItem?.lastValueStart != null
          ? _degreeToRad(_radToDegree(oldItem?.lastValueStart ?? 0.0) - 360.0)
          : 0.0,
